BAHASA INDONESIA (ID):

!!!⚠️ PERINGATAN - PROYEK INI SEMATA-MATA DITUJUKAN UNTUK KEPERLUAN AKADEMIK DAN TUGAS PERKULIAHAN. PT TELKOMSEL PADA PROYEK INI HANYA DIGUNAKAN SEBAGAI CONTOH PERUSAHAAN HIPOTESIS DALAM STUDI KASUS INI. ⚠️!!!

TERA adalah aplikasi web yang menggabungkan tempat berbagi ilmu (Knowledge Management System) dengan sistem pembelajaran pintar (Adaptive Learning Engine). Aplikasi ini dibuat untuk mengumpulkan, mengelola, dan membagikan semua pengalaman serta panduan perbaikan (troubleshooting) dari para teknisi jaringan Telkomsel di lapangan secara langsung (real-time).

Untuk Menjalankan TERA:
1. Install Komponen Terlebih Dahulu:
   "npm install"
2. Menjalankan Website TERA:
   "npm run dev`"

Akun:
1. Akun Teknisi:
   * e-mail: sutan@tera.com
   * Kata Sandi: sutan
2. Akun Admin:
   * e-mail: admin@tera.com
   * Kata Sandi: admin

ENGLISH (ENG):

!!!⚠️ WARNING - THIS PROJECT IS STRICTLY FOR ACADEMIC AND UNIVERSITY COURSEWORK PURPOSES. PT. TELKOMSEL IS USED SOLELY AS A HYPOTHETICAL COMPANY EXAMPLE FOR THIS CASE STUDY. ⚠️!!!

TERA is a web-based application that combines a Knowledge Management System (KMS) with an Adaptive Learning Engine. This platform is designed to collect, manage, and share real-time field experiences and troubleshooting guides among network technicians.

ForRunning TERA:
1. Install Dependencies:
   "npm install"
2. Start the TERA Website:
   "npm run dev"

Accounts:
1. Technician Account:
   * Email: sutan@tera.com
   * Password: sutan
2. Administrator Account
   * Email: admin@tera.com
   * Password: admin

Tangkapan Layar - Screenshot:
1. Halaman Masuk - Log in Page
<img width="1864" height="946" alt="opera_CsIxj2wcjQ" src="https://github.com/user-attachments/assets/f17d927d-2703-4095-a181-633f94aa224f" />

2. Halaman Daftar - Sign up Page
<img width="1864" height="946" alt="opera_U8NV57HHAz" src="https://github.com/user-attachments/assets/573f669b-e5bc-49ee-8a5a-6840428553fc" />

3. Halaman Dashboard Teknisi - Technician Dashboard Page
<img width="1864" height="946" alt="opera_UGZ5m8x1t2" src="https://github.com/user-attachments/assets/eadd0090-0d17-4628-9e76-f13418ef9965" />
<img width="1864" height="946" alt="opera_l5yyIYwxcU" src="https://github.com/user-attachments/assets/6eb7ef5c-a931-45d6-b76e-d7ff43365e48" />
<img width="1864" height="946" alt="opera_fYe2bROfwd" src="https://github.com/user-attachments/assets/127affa1-e5e0-4d6d-9c15-535cd3f6d9c4" />
<img width="1864" height="946" alt="opera_qBXgy2F8aZ" src="https://github.com/user-attachments/assets/d1e01450-488f-4da0-bae4-22ea31f0f258" />
<img width="1864" height="946" alt="opera_SHeRirJO02" src="https://github.com/user-attachments/assets/06bab581-2214-4826-97c6-05aa3aa43c38" />
<img width="1864" height="946" alt="opera_tgxJiRUMmH" src="https://github.com/user-attachments/assets/7b9ac6c0-069a-45e3-9fa2-4d4f37b34c7c" />
<img width="1864" height="946" alt="opera_VaR7GUfwNp" src="https://github.com/user-attachments/assets/c1ae071a-bb90-49f1-b5f7-c616f73d493e" />
<img width="1864" height="946" alt="opera_IZgxefdOsN" src="https://github.com/user-attachments/assets/ecfb4ed6-0d6d-4cc8-b5bb-6b3edbcd3ec1" />
<img width="1864" height="946" alt="opera_2mOWnKaPbz" src="https://github.com/user-attachments/assets/f61542e8-a1ad-44c3-84ad-5d3b411aabc6" />
<img width="1864" height="946" alt="opera_XsBKnfhNuX" src="https://github.com/user-attachments/assets/c6983266-2583-4f28-8922-3f9805854091" />
<img width="1864" height="946" alt="opera_d816fgNfOW" src="https://github.com/user-attachments/assets/d4c27039-b9a0-48c9-b90f-86645e2a0078" />
<img width="1864" height="946" alt="opera_8pGBtxXgvy" src="https://github.com/user-attachments/assets/25e0a92b-c394-4fde-8984-40328634ea0b" />
<img width="1864" height="946" alt="opera_MdYX6SGTEr" src="https://github.com/user-attachments/assets/1cc1db4b-cfc6-4b3f-8008-99459ec32211" />

4. Halaman Dashboard Admin - Admin Dasboard Page
<img width="1864" height="946" alt="opera_JZGKzFfx4d" src="https://github.com/user-attachments/assets/8ca7c878-5510-4f6f-8bd7-33a352df7d74" />
<img width="1864" height="946" alt="opera_A1xxewguGW" src="https://github.com/user-attachments/assets/e19a28a0-05c2-43f3-a7b1-7e8b31d462ab" />
<img width="1864" height="946" alt="opera_CkRMlVUkpD" src="https://github.com/user-attachments/assets/7afe193c-4565-4638-bffc-125abcb44e2e" />
<img width="1864" height="946" alt="opera_JilPvzXfJV" src="https://github.com/user-attachments/assets/db0fed68-f91f-46fd-a719-af58942423c8" />
<img width="1864" height="946" alt="opera_y9xp5vQByi" src="https://github.com/user-attachments/assets/2ea47789-acfa-4a67-973d-a60600234bf0" />
